...
export default function Workouts({ user, profile }) {
  ...
  const toggleReaction = async (resultId, type, hasReacted) => {
    if (hasReacted) {
      ...
      // One reaction per athlete per result. Choosing a new emoji replaces the old one.
      await supabase.from('reactions').delete().match({ result_id: resultId, athlete_id: user.id })
        ...
        <div>
          <div className="workout-body">
            ...
            {sections.map(sec => {
              ...
              const movementLeaderboards = scoreType === 'Heaviest Set' ? buildMovementLeaderboards(sec) : []
              const sectionLeaderboard = scoreType !== 'No Score' && scoreType !== 'Heaviest Set' ? buildSectionLeaderboard(sec, scoreType) : []
              ...
              return (
                <div key={sec.id} className="section-block">
                  ...
                  {/* Movement-by-movement leaderboards for strength sections */}
                  {scoreType === 'Heaviest Set' && movementLeaderboards.map(board => (
                    <SectionLeaderboard
                      key={board.movementId}
                      entries={board.entries}
                      scoreType={board.title}
                      userId={user.id}
                      reactions={RX}
                  ...
                  {/* Per-section leaderboard for conditioning/scored sections */}
                  {scoreType !== 'No Score' && scoreType !== 'Heaviest Set' && sectionLeaderboard.length > 0 && (
                    <SectionLeaderboard
  ...
  return (
    <div className="leaderboard" style={{ marginTop: '12px' }}>
      ...
      {entries.map((entry, i) => {
        ...
        const entryKey = entry.entryKey || `${entry.athleteId}-${entry.sectionId}`
        const isExpanded = expandedAthlete === entryKey
        ...
        const myReaction = reactions.find(rx => allReactions.some(x => x.type === rx.k && x.athlete_id === userId))
        const pickerOpen = openPicker === entryKey

        return (
          <div key={entryKey}>
            <div className="lb-row" style={{ cursor: 'pointer', position: 'relative' }} onClick={() => setExpandedAthlete(isExpanded ? null : entryKey)}>
              <span className={`lb-rank ${rankClass}`}>{rankSym}</span>
              ...
              {result && (
                <div className="lb-reactions" style={{ position: 'relative' }} onClick={e => e.stopPropagation()}>
                  <button
                    className={`reaction-btn ${myReaction ? 'reacted' : ''}`}
                    onClick={() => setOpenPicker(pickerOpen ? null : entryKey)}
                    title="React"
                  >
                    {myReaction ? myReaction.e : '✨'}{totalReactions > 0 && <span>{totalReactions}</span>}
                  </button>
                  ...
                  {pickerOpen && (
                    <div style={{
                      ...
                      {reactions.map(rx => {
                        const rxArr = allReactions.filter(x => x.type === rx.k)
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              onToggleReaction(result.id, rx.k, hasReacted)
                              setOpenPicker(null)
                            }}
                            title={rx.k}
                          >
...
function buildMovementLeaderboards(section) {
  return (section.movements || [])
    ...
    .map(mov => {
        ...
        ;(st.set_logs || []).forEach(sl => {
          ...
          if (!athleteMap[sl.athlete_id]) {
            athleteMap[sl.athlete_id] = {
              ...
              movementId: mov.id,
              entryKey: `${sl.athlete_id}-${section.id}-${mov.id}`,
      ...
      const entries = Object.values(athleteMap)
        .map(a => {
          const bestScore = getBestScore(a.allValues, 'Heaviest Set')
          return {
            ...a,
            displayScore: bestScore,
            sortScore: parseScore(bestScore, 'Heaviest Set')
          }
        })
        .filter(a => a.displayScore !== null)
        .sort((a, b) => b.sortScore - a.sortScore)

      return { movementId: mov.id, title: mov.name, entries }
    })
