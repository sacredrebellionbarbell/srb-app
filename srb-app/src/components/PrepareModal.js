...
export default function PrepareModal({ workout, user, onClose }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      const { data } = await supabase
        .from('results')
        .select('*, workouts(title, date)')
        .eq('athlete_id', user.id)
        .order('created_at', { ascending: false })
      setMyResults((data || []).filter(r => xWeight(r.score) !== null))
    }
    fetch()
  }, [user])
  ...
  const est = useMemo(() => {
    ...
    let best = 0
    myResults.forEach(r => { const e = epley(xWeight(r.score), xReps(r.note)); if (e > best) best = e })
          ...
          {est
            ? <>
                <div className="est-box">
                  <div className="est-label">Estimated 1RM</div>
            ...
            : <div className="est-box">
                <div className="est-label">No estimate yet</div>
                <div style={{ fontSize: '13px', color: 'var(--charcoal-light)', marginTop: '6px' }}>
                  Enter a weight and rep count above, or log some weight-based results first.
                </div>
              </div>
          }

          <div className="past-title">Your Past Results</div>
          {myResults.length === 0
            ? <p style={{ fontSize: '13px', color: 'var(--charcoal-light)' }}>No weight-based results yet.</p>
            : myResults.map((r, i) => (
