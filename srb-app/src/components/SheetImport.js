...
function parseSetScheme(value) {
  const raw = clean(value)
  if (!raw) return { count: 0, reps: '', load: '', note: '' }
  ...
  if (countMatch) {
    return { count: 1, reps: countMatch[1], load: '', note: clean(countMatch[2]) }
  }
...
function buildSetsFromScheme(schemeText) {
  const scheme = parseSetScheme(schemeText)
  if (!scheme.count) return []
  return Array.from({ length: Math.min(scheme.count, 20) }, (_, index) => ({
    set_number: index + 1,
    reps: scheme.reps,
    load: scheme.load,
    rpe: '',
...
function parseGenericSetPlan(row) {
...
function dayNumber(dayText) {
  const match = clean(dayText).match(/(\d+)/)
  if (!match) return 1
  return Math.max(1, Math.min(3, parseInt(match[1], 10)))
}

function dateForProgramRow(row, startDate) {
...
function isStrongSavageFormat(rows) {
  ...
  return Boolean(
    ...
    first.day != null &&
    first.warm_up != null &&
    first.main_lift != null &&
    first.sets_reps != null
  )
}

function buildStrongSavageWorkouts(rows, options) {
  const track = options.track || 'Strong & Savage'
  return rows.map(row => {
    const week = pick(row, ['week']) || '1'
    const day = dayNumber(pick(row, ['day']))
    const mainLift = pick(row, ['main_lift']) || 'Main Lift'
    const schemeText = pick(row, ['sets_reps'])
    const scheme = parseSetScheme(schemeText)
    const accessory = pick(row, ['accessory'])
    const circuit = pick(row, ['strongman_circuit', 'conditioning'])
    const coachingNotes = pick(row, ['coaching_notes'])
    const warmUp = pick(row, ['warm_up'])
    const title = `Week ${week} Day ${day} - ${mainLift}`
    const mainNotes = [scheme.note, coachingNotes].filter(Boolean).join(' | ')
    ...
    sections.push({
      ...
      movements: [{
        ...
        notes: scheme.note,
        demo_url: '',
        sets: buildSetsFromScheme(schemeText)
      }]
    })
    ...
    if (accessory) {
      sections.push({
        ...
        movements: [{
          ...
          notes: '',
          demo_url: '',
          sets: [{ set_number: 1, reps: accessory, load: '', rpe: '', order_index: 0 }]
        }]
      })
    }
    ...
    if (circuit) {
      sections.push({
        type: 'Conditioning',
        score_type: circuit.toLowerCase().includes('amrap') ? 'AMRAP' : circuit.toLowerCase().includes('for time') ? 'For Time' : 'No Score',
        notes: circuit,
        movements: []
      })
    }
    ...
    return {
      title,
      date: dateForProgramRow(row, options.startDate),
      track,
      notes: coachingNotes,
...
function buildGenericWorkouts(rows, members) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    const nextSets = parseGenericSetPlan(row)
...
export default function SheetImport() {
  ...
  const [track, setTrack] = useState('Strong & Savage')
  ...
  const loadFile = async (event) => {
    ...
    setRawText(text)
    const inferred = inferStartDateFromName(file.name)
    if (inferred) setStartDate(inferred)
  }
    ...
    <div>
      ...
      <div className="panel">
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '1rem' }}>
          ...
          <button className="btn-ghost" onClick={() => { setRawText(TEMPLATE); setFileName('Template') }}>Load Detailed Template</button>
          <button className="btn-ghost" onClick={() => { setRawText(''); setFileName(''); setErrors([]) }}>Clear</button>
        </div>

        {fileName && (
          <div style={{ fontSize: '12px', color: 'var(--moss-light)', marginBottom: '1rem' }}>
            Loaded: {fileName}
          </div>
        )}
          ...
          <div className="two-col">
            <div className="field">
              ...
              <select value={track} onChange={e => setTrack(e.target.value)}>
                {TRACKS.filter(t => t !== 'Private').map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
            <div className="field">
              <label>Week 1 Day 1 Date</label>
        ...
        <div className="field">
          ...
          <textarea
            ...
            placeholder="Upload a CSV or paste from Google Sheets here..."
                ...
                <div className="workout-header" style={{ cursor: 'default' }}>
                  <div>
                    ...
                    <div className="workout-meta">
                      <span className={`track-badge ${workout.track === 'Strong & Savage' ? 'track-strength' : 'track-open'}`}>{workout.track}</span>
                      <span className="future-badge">{workout.date || 'Choose start date'}</span>
                    </div>
                  </div>
                </div>
                <div className="workout-body">
                  {workout.sections.map((section, sectionIndex) => (
                    <div key={sectionIndex} className="section-block">
                      ...
                      {section.movements.length === 0 && <p className="no-data" style={{ padding: 0 }}>No movement rows needed for this section.</p>}
                      {section.movements.map((movement, movementIndex) => (
                        <div key={movementIndex} className="movement-block">
                          ...
                          <div style={{ fontSize: '13px', color: 'var(--charcoal-light)' }}>
                            ...
                            {movement.sets[0]?.reps ? ` · ${movement.sets[0].reps}` : ''}
